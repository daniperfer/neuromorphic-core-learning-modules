...
print("Running gamma oscillation simulation...")
run(duration)
...
t_E, rate_E = compute_population_rate(spike_E, N_E, bin_size_ms, duration / ms)
t_I, rate_I = compute_population_rate(spike_I, N_I, bin_size_ms, duration / ms)
...
print(f"Dominant oscillation frequency — Excitatory: {peak_E:.1f} Hz")
print(f"Dominant oscillation frequency — Inhibitory: {peak_I:.1f} Hz")

fig, axes = plt.subplots(4, 1, figsize=(14, 12))

# ── Raster (first 500 ms for clarity) ────────────────────────
t_max_plot = 500  # ms
mask_e = spike_E.t / ms < t_max_plot
mask_i = spike_I.t / ms < t_max_plot

axes[0].scatter((spike_E.t / ms)[mask_e], spike_E.i[mask_e], s=0.8, color="steelblue", alpha=0.5)
axes[0].scatter(
    (spike_I.t / ms)[mask_i], spike_I.i[mask_i] + N_E, s=0.8, color="crimson", alpha=0.5
)
...
for idx in range(N_I):
    mask = spike_I.i == idx
    spike_trains_I[idx] = np.array(spike_I.t[mask] / second)
