...
if __name__ == "__main__":
    ...
    print(
        f"Neuron A: {len(spike_times_A)} spikes  ({len(spike_times_A) / recording_duration:.1f} Hz)"
    )
    print(
        f"Neuron B: {len(spike_times_B)} spikes  ({len(spike_times_B) / recording_duration:.1f} Hz)"
    )
    print()
    ...
    lags_AB, ccg_AB = cross_correlogram(
        spike_times_A, spike_times_B, max_lag_ms=50.0, bin_width_ms=0.5
    )
